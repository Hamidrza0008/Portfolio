      ...
      <div className="max-w-5xl mx-auto space-y-7">
        {techStack.map((group, gIdx) => (
          <div key={gIdx} className="space-y-3">
            <h2 className="text-sm md:text-base font-semibold text-gray-300 border-l-4 border-orange-500 pl-3 tracking-wide uppercase">
              {group.category}
            </h2>
            ...
            <div className="flex flex-wrap gap-2.5">
              {group.items.map((skill, sIdx) => (
                <div
                  key={sIdx}
                  className="skill-card group flex items-center gap-2 px-3.5 py-2 bg-white/5 backdrop-blur-md rounded-full border border-white/10 transition-all duration-300 hover:scale-105 hover:border-orange-500/50 hover:bg-white/10 hover:shadow-[0_0_15px_rgba(249,115,22,0.2)]"
                >
                  {skill.img ? (
                    <img
                      src={skill.img}
                      alt={skill.label}
                      className="h-4 w-4 object-contain opacity-80 group-hover:opacity-100 transition-all duration-300 invert"
                    />
                  ) : (
                    <i
                      className={`fa-${skill.solid ? "solid" : "brands"} ${skill.icon} text-sm text-orange-500/80 group-hover:text-orange-500 transition-all duration-300`}
                    ></i>
                  )}
                  <span className="text-xs sm:text-sm font-medium text-gray-400 group-hover:text-orange-400 transition-colors duration-300 whitespace-nowrap">
                    {skill.label}
                  </span>
                </div>
              ))}
